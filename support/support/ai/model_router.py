...
_BACKEND_DEFAULTS: dict[str, dict[str, str]] = {
    ...
    # Note: Claude uses Anthropic API (not OpenAI-compatible).
    # Use an OpenAI-compatible proxy or the dedicated AnthropicBackend instead.
    # "claude": {
...
class ModelRouter:
    ...
    def from_config(cls, config: dict) -> ModelRouter:
        """Build router from config dict (config.yaml ai section).

        Config format:
            ai:
              backend: minimax          # default backend for all tasks
              api_key: sk-xxx
              base_url: https://...     # optional
              model: MiniMax-Text-01    # optional
              backends:                 # optional additional backends
                deepseek:
                  api_key: sk-yyy
                  model: deepseek-chat
              routing:                  # optional task→backend mapping
                translate: minimax
                detect_lang: minimax
                ai_reply: minimax
                summarize: minimax
        ...
        # Load all default backends from env first
        for name, defaults in _BACKEND_DEFAULTS.items():
            api_key = os.environ.get(defaults["api_key_env"], "")
            base_url = os.environ.get(defaults["base_url_env"], defaults["base_url_default"])
            model = os.environ.get(defaults["model_env"], defaults["model_default"])
        ...
        # Override/set the primary backend from config
        primary = config.get("backend", "minimax")
        primary_key = config.get("api_key", "")
        if primary_key and primary in backends:
            backends[primary] = BackendConfig(
                name=primary,
                base_url=config.get("base_url", backends[primary].base_url),
                api_key=primary_key,
                model=config.get("model", backends[primary].model),
            )

        # Additional backends from config
        extra = config.get("backends", {})
        for name, bcfg in extra.items():
            defaults = _BACKEND_DEFAULTS.get(name, {})
            backends[name] = BackendConfig(
                name=name,
                base_url=bcfg.get("base_url", defaults.get("base_url_default", "")),
                api_key=bcfg.get("api_key", ""),
                model=bcfg.get("model", defaults.get("model_default", "")),
            )

        # Task routing: config overrides, then env, then defaults (all → primary)
        routing_cfg = config.get("routing", {})
        task_routing: dict[str, str] = {}
        for task in ALL_TASKS:
            if task in routing_cfg:
                task_routing[task] = routing_cfg[task]
            else:
                env_key = f"CS_ROUTER_{task.upper()}"
                task_routing[task] = os.environ.get(env_key, primary)

        return cls(backends=backends, task_routing=task_routing)
